...
function renderUserList(data) {
  ...
  data.forEach((item) => {
    rawHTML += `<div class="card col-2">
      ...
      <button id="addToFavorite" type="button" class="btn btn-info col-2" data-id="${item.id}">+</button>
      </div>
    </div>`
  });
...
axios
  ...
  .then((response) => {
    ...
    localStorage.setItem("NEW", JSON.stringify(users))
  })
...
function ageChanged() {
  ...
  if (Number(ageChange.value) !== 0) {
    ...
    localStorage.setItem("NEW", JSON.stringify(newForm))
...
Fav.addEventListener("click", e => {
  window.location = 'http://127.0.0.1:5500/%E7%A4%BE%E7%BE%A4/favorite.html'
})
...
paginator.addEventListener('click', function onPaginatorClicked(event) {
  ...
  const filterData = JSON.parse(localStorage.getItem("NEW"))
...
dataPanel.addEventListener('click', function onPanelClicked(event) {
  if (event.target.matches('#addToFavorite')) {
